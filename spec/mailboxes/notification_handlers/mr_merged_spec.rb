...
RSpec.describe NotificationHandlers::MrMerged do
  ...
  describe '.matches?' do
    context 'with a merged fixture' do
      let(:mail) { receive_inbound_email_from_fixture('merged.eml').mail }
  ...
  describe 'merged.eml – merge notification' do
    ...
    it 'includes the action phrase in the summary' do
      inbound_email
      expect(user.notifications.last.summary).to include('merged')
    end

    it 'includes the author in the summary' do
      inbound_email
